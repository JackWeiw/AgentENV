...
/// When set (to any non-empty value), record tracing span durations to a
/// Chrome / Perfetto trace file (open in `chrome://tracing` or ui.perfetto.dev).
...
/// - Set `AENV_CHROME_TRACE=1` (any non-empty value) to additionally record span
///   durations to a Chrome/Perfetto trace file for profiling pause/resume under
///   load. The file is written as `trace-chrome-*.json` in the server's cwd;
///   the writer runs for the process lifetime and is intentionally leaked so it
///   keeps draining; unset = fully disabled with zero overhead.
///
...
pub fn init() {
    INIT_LOGGING.call_once(|| {
        ...
        if std::env::var(CHROME_TRACE_ENV).map_or(false, |v| !v.trim().is_empty()) {
            // Uses tracing-chrome's default writer, which writes
            // `trace-chrome-<timestamp>.json` into the server's cwd. The env
            // var is treated as an enable flag (its value is ignored).
            ...
            eprintln!(
                "AENV_CHROME_TRACE enabled: writing trace-chrome-*.json to the server cwd"
            );
        } else {
